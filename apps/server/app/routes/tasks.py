...
@tasks_bp.route('/schedules', methods=['GET'])
def get_schedules():
    """
    Get the most recent generated daily schedules.
    Returns schedules that were pre-generated by Lambda.
    """
    try:
        # Get the most recent schedule
        latest_schedule = Schedule.query.order_by(Schedule.generated_date.desc()).first()
        
        if not latest_schedule:
            return jsonify({
                'status': 'error',
                'message': 'No schedules available yet. They will be generated at 7 AM daily.'
            }), 404
        
        return jsonify({
            'status': 'success',
            'data': latest_schedule.to_dict()
        }), 200
        
    except Exception as e:
        logging.exception("Error fetching schedules")
        return jsonify({
            'status': 'error',
            'message': f'Error retrieving schedules: {str(e)}'
        }), 500

@tasks_bp.route('/schedules', methods=['POST'])
def save_schedules():
    """
    Save schedules generated by Lambda.
    This endpoint is called BY Lambda after generating schedules.
    """
    try:
        data = request.get_json()
        ...
        new_schedule = Schedule(
            generated_date=datetime.fromisoformat(data.get('date', datetime.utcnow().isoformat())),
            task_count=data.get('task_count', 0),
            research_data=json.dumps(data.get('research', [])),
            schedules_data=json.dumps(data.get('schedules', []))
        )
        
        db.session.add(new_schedule)
        db.session.commit()
        
        logging.info(f"Saved new schedule with {data.get('task_count')} tasks")
        ...
        logging.exception("Error saving schedules")
        db.session.rollback()
        return jsonify({
            'status': 'error',
            'message': f'Failed to save schedules: {str(e)}'
        }), 500
